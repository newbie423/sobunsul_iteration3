...
public class BorrowerDB{
    ...
    // 이용자DB의 iterator를 생성한뒤 매번 호출될 때마다 다음요소를 전달하며,
    // 모든 이용자에 대한 순회가 끝난 경우 iterator를 다시 초기화 하는 메소드이다
    public Borrower getOneBorrower(){
        ...
        if(it.hasNext()){ // 다음 이용자가 있을때만, 다음 이용자를 받아오기
            borrower = this.it.next();
        }else{ // 
            this.it = null; // 다음 이용자가 없으면 현재 iterator에 대한 모든 이용자를 다 순회한 것이므로 다음번 사용을 위해 초기화
        }
